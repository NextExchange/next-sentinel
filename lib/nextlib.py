...
def is_valid_next_address(address, network='mainnet'):
    ...
    # Support next address (T-address on testnet and X-address on mainnet)
    next_version = 65 if network == 'testnet' else 75
    ...
    try:
        ...
        # rescue from exception, not a valid Nextcoin address
        return False

    if (address_version != next_version and address_version != bitcoin_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.nextd_serialise())
...
def SHIM_serialise_for_nextd(sentinel_hex):
    ...
    # superblock => "trigger" in nextd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # nextd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    nextd_hex = serialise(obj)
    return nextd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('next.conf')
