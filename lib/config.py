...
sentinel_cfg = NextcoinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.2"
min_nextd_proto_version_with_sentinel_ping = 70221


def get_next_conf():
    if sys.platform == 'win32':
        next_conf = os.path.join(os.getenv('APPDATA'), "NextcoinCore/next.conf")
    else:
        home = os.environ.get('HOME')

        next_conf = os.path.join(home, ".next/nextcoin.conf")
        if sys.platform == 'darwin':
            next_conf = os.path.join(home, "Library/Application Support/NextcoinCore/next.conf")

    next_conf = sentinel_cfg.get('next_conf', next_conf)
