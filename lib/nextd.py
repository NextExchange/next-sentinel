...
class NextcoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some nextd methods
    ...
    @classmethod
    def from_next_conf(self, next_dot_conf):
        from next_config import NextcoinConfig
        config_text = NextcoinConfig.slurp_config_file(next_dot_conf)
        creds = NextcoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = nextlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = nextlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = nextlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_nextd_proto_version_with_sentinel_ping)
