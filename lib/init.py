...
def has_next_conf():
    ...
    valid_next_conf = False

    # ensure next_conf exists & readable
    #
    # if not, print a message stating that Nextcoin Core must be installed and
    # configured, including JSONRPC access in next.conf
    try:
        f = io.open(config.next_conf)
        valid_next_conf = True
    except IOError as e:
        print(e)

    return valid_next_conf
...
def main():
    ...
    if not has_next_conf():
        print("NextcoinCore must be installed and configured, including JSONRPC access in next.conf")
        sys.exit(1)
