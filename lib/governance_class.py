...
class GovernanceClass(object):
    ...
    def vote(self, nextd, signal, outcome):
        return self.go.vote(nextd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, nextd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(nextd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(nextd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import nextlib
        obj_data = nextlib.SHIM_serialise_for_nextd(self.serialise())
    ...
    def nextd_serialise(self):
        import nextlib
        return nextlib.SHIM_serialise_for_nextd(self.serialise())
