...
@pytest.fixture
def valid_next_address(network='mainnet'):
    return 'TSFMFYoPjz9xg9fUYXxxWEeEDCQ3qSH1aJ' if (network == 'testnet') else 'TJE7PNc64yXpH5eGVVzzn5F6xzAF8ZxN6x'


@pytest.fixture
def invalid_next_address(network='mainnet'):
    return 'TSFMFYoPjz9xg9fUYXxxWEeEDCQ3qSH1aK' if (network == 'testnet') else 'TA7Pf8mXyAGRV8aGVSZwrTH96NjvyA9qDC'
...
def test_valid_next_address():
    from nextlib import is_valid_next_address

    main = valid_next_address()
    test = valid_next_address('testnet')
    main_btc = valid_bitcoin_address()
    test_btc = valid_bitcoin_address('testnet')

    assert is_valid_next_address(main) is True
    assert is_valid_next_address(main, 'mainnet') is True
    assert is_valid_next_address(main, 'testnet') is False

    assert is_valid_next_address(test) is False
    assert is_valid_next_address(test, 'mainnet') is False
    assert is_valid_next_address(test, 'testnet') is True

    assert is_valid_next_address(main_btc) is True
    assert is_valid_next_address(main_btc, 'mainnet') is True
    assert is_valid_next_address(main_btc, 'testnet') is False

    assert is_valid_next_address(test_btc) is False
    assert is_valid_next_address(test_btc, 'mainnet') is False
    assert is_valid_next_address(test_btc, 'testnet') is True


def test_invalid_next_address():
    from nextlib import is_valid_next_address

    main = invalid_next_address()
    test = invalid_next_address('testnet')
    main_btc = invalid_bitcoin_address()
    test_btc = invalid_bitcoin_address('testnet')

    assert is_valid_next_address(main) is False
    assert is_valid_next_address(main, 'mainnet') is False
    assert is_valid_next_address(main, 'testnet') is False

    assert is_valid_next_address(test) is False
    assert is_valid_next_address(test, 'mainnet') is False
    assert is_valid_next_address(test, 'testnet') is False

    assert is_valid_next_address(main_btc) is False
    assert is_valid_next_address(main_btc, 'mainnet') is False
    assert is_valid_next_address(main_btc, 'testnet') is False

    assert is_valid_next_address(test_btc) is False
    assert is_valid_next_address(test_btc, 'mainnet') is False
    assert is_valid_next_address(test_btc, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from nextlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % nextlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(nextlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(nextlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(120).quantize(precision)
    assert int(nextlib.blocks_to_seconds(16616)) == 996960
